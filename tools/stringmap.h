...
public:
  typedef Val     TypeVal;
  typedef ArgVal  TypeArgVal;
public:
                stringmap( unsigned tablen = 69959 );
               ~stringmap();
// Map work methods
  int           Delete( const char*, unsigned l = (unsigned)-1 );
  void          DelAll();
  unsigned      GetLen() const;
  const char*   GetStr( const char*, unsigned l = (unsigned)-1 ) const;
  unsigned      TabLen() const;
  int           Insert( const char*, ArgVal );
  int           Insert( const char*, ArgVal, Val*& );
  Val*          Insert( const char* );
  Val*          Search( const char*, unsigned l = (unsigned)-1 ) const;
...
protected:
  ...
  unsigned  gethash( const char* psz, unsigned cch ) const
              {
                unsigned  int nHash = 0;

                assert( cch != (unsigned)-1 );
                while ( cch-- > 0 )
                  nHash = (nHash << 5) + nHash + (unsigned char)*psz++;
                return nHash;
              }
  bool      iseq( const char* p1, const char* p2, unsigned cc ) const
              {
                assert( cc != (unsigned)-1 );
...
template <class Val, class ArgVal>
inline  int   stringmap<Val, ArgVal>::Delete( const char* k, unsigned l )
{
  if ( l == (unsigned)-1 )
  ...
  if ( pitems != NULL && ncount != 0 )
  {
    ...
    while ( *ppitem != NULL && !iseq( (*ppitem)->key, k, l ) )
      ppitem = &(*ppitem)->lpn;
    if ( *ppitem != NULL )
    {
      lpfree = *ppitem;
...
template <class Val, class ArgVal>
inline  const char* stringmap<Val, ArgVal>::GetStr( const char* k, unsigned l ) const
{
  if ( l == (unsigned)-1 )
    for ( l = 0; k[l] != '\0'; ++l ) (void)NULL;
  ...
  if ( pitems != NULL )
  {
    ...
    while ( lpitem != NULL && !iseq( lpitem->key, k, l ) )
      lpitem = lpitem->lpn;
    return ( lpitem != NULL ? lpitem->key : NULL );
  }
    else
  return NULL;
}
...
template <class Val, class ArgVal>
inline  Val*  stringmap<Val, ArgVal>::Search( const char* k, unsigned l ) const
{
  if ( l == (unsigned)-1 )
    for ( l = 0; k[l] != '\0'; ++l ) (void)NULL;
